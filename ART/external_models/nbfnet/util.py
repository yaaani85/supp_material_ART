...
def build_solver(cfg, dataset):
    ...
    cfg.gpus = [0]
    solver = core.Engine(task, train_set, valid_set, test_set, optimizer, None, cfg.gpus)
    print("solver created")
    if "checkpoint" in cfg:
        print("loading checkpoint")
        try:
            solver.load(cfg.checkpoint, strict=False)
        except AttributeError as e:
            if "is_meta" in str(e):
                print("Ignoring is_meta attribute error - continuing with partial load")
            else:
                raise e
